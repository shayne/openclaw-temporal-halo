...
export function buildDreamInstructions(cfg: TemporalHaloConfig): string {
	return [
		...
		"You MUST run dream work in a subagent when possible.",
		"HALO.md MUST prioritize high-value life context that helps resolve ambiguous user requests without extra back-and-forth.",
		"",
		"Execution contract:",
		"1) You MUST call `sessions_spawn` before any source retrieval.",
		"2) The spawned task MUST perform the full dream workflow below and MUST publish via `temporal_halo_publish`.",
		"3) In this parent session, do not gather sources and do not call `temporal_halo_publish`.",
		"4) After a successful spawn, stop.",
		"5) If `sessions_spawn` is unavailable/forbidden/fails, run the workflow below inline in this session.",
		"",
		"Dream workflow (worker or inline fallback):",
