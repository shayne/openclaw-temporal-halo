...
export function buildDreamInstructions(cfg: TemporalHaloConfig): string {
	return [
		...
		"",
		"Cron reminder override:",
		"- If the base prompt says 'A scheduled reminder has been triggered' or asks you to relay a reminder, treat that as a wake trigger only.",
		"- Do not send a reminder-style reply.",
		"- This run is maintenance-only: gather data, refresh HALO.md, and publish.",
		"- You must call `temporal_halo_publish` before finishing this run.",
		"",
